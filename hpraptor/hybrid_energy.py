...
class HybridMissionResult:
    ...
    def total_energy_wh(self) -> float:
        """Total energy consumed from all sources [Wh]."""
        return self.total_battery_energy_wh + sum(
            s.fuel_consumed_kg * 43e6 / 3600 for s in self.segments  # Approximate
        )
...
class HybridEnergyManager:
    ...
    def analyze_path(self, path, power_schedule: Dict = None,
        ...
        for i, seg in enumerate(path.segments):
            ...
            bat_result = self.battery.discharge(P_from_battery, k.duration) if P_from_battery > 0 else self.battery._null_result()
            ...
            if mode == "hybrid_charge" and fuel_flow > 0:
                ...
                if P_excess > 0:
                    charge_result = self.battery.charge(P_excess, k.duration)
                    charged_wh = charge_result.get('energy_wh_stored', 0)

            # Fuel consumption
            fuel_result = self.fuel.consume(fuel_flow, k.duration)
